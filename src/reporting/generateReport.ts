...
): { success: boolean; paths?: { lightTheme: string; darkTheme: string }; message: string } {
  try {
    ...
    const inputPath = cucumberJsonPath || reportingConfig.cucumberJsonPath || './coverage/functional-tests/cucumber-report.json';
    ...
    const results = generateDashboardFromCucumberJson(inputPath, outputDir, themeConfigs);
...
export function autoGenerateDashboard(): void {
  ...
  if (dashboardConfig.autoGenerate) {
    ...
    if (result.success) {
      ...
      console.warn('⚠️ Dashboard auto-generation failed:', result.message);
    }
  }
}
