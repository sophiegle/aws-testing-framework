...
Then(
  ...
  async function (this: StepContext, fileName: string) {
    ...
    if (!trace) {
      throw new Error(
        `Could not trace file ${fileName} through the entire pipeline`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (!trace?.sqsMessage || !trace?.lambdaExecution) {
      throw new Error(
        'Lambda execution does not correlate with the SQS message'
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (!trace?.lambdaExecution || !trace?.stepFunctionExecution) {
      throw new Error(
        'Step Function execution does not correlate with the Lambda execution'
      );
    }
  }
);
