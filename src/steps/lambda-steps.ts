...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (!this.correlationId) {
      throw new Error(
        'Correlation ID is not set. Make sure to upload a file first.'
      );
    }

    const lambdaExecuted = await framework.trackLambdaExecution(
      this.functionName,
      this.correlationId
    );
    if (!lambdaExecuted) {
      throw new Error(
        'Lambda function did not consume the SQS message within timeout'
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext, fileName: string) {
    ...
    const processedFile = await framework.verifyLambdaProcessedFile(
      this.correlationId,
      fileName
    );
    if (!processedFile) {
      throw new Error(
        `Lambda function did not process the exact file ${fileName}`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext, stateMachineName: string) {
    ...
    const stepFunctionTriggered = await framework.trackStepFunctionExecution(
      stateMachineName,
      this.correlationId
    );
    if (!stepFunctionTriggered) {
      throw new Error(
        `Lambda function did not trigger the Step Function ${stateMachineName} within timeout`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext, timeWindowMinutes: number) {
    ...
    const hasErrors = await framework.checkLambdaErrors(
      this.functionName,
      timeWindowMinutes
    );
    if (hasErrors) {
      throw new Error(
        `Lambda function ${this.functionName} has errors in the last ${timeWindowMinutes} minutes`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext, maxExecutionTimeMs: number) {
    ...
    const executionTimeOk = await framework.verifyLambdaExecutionTime(
      this.functionName,
      maxExecutionTimeMs
    );
    if (!executionTimeOk) {
      throw new Error(
        `Lambda function ${this.functionName} execution time exceeds ${maxExecutionTimeMs}ms`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (issues.length > 0) {
      throw new Error(
        `Lambda function ${this.functionName} configuration issues: ${issues.join(', ')}`
      );
    }
  }
);
