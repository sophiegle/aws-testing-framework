...
When(
  'I start an execution with input {string}',
  async function (this: StepContext, input: string) {
    ...
    this.executionArn = await framework.startExecution(
...
Then(
  'the Step Function should be triggered',
  async function (this: StepContext) {
    ...
    await framework.waitForCondition(async () => {
      ...
      if (stateMachineTriggered && !this.executionArn) {
        ...
        const executions = await framework.listExecutions(
          this.stateMachineName
        );
...
Then(
  'the Step Function should be executed multiple times',
  async function (this: StepContext) {
    if (!this.stateMachineName) {
      throw new Error(
        'State machine name is not set. Make sure to create a Step Function first.'
