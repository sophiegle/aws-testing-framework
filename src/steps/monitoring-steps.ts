...
Then(
  ...
  async function (this: StepContext, expectedPattern: string) {
    ...
    if (!result.found) {
      throw new Error(
        `Lambda logs do not contain pattern: ${expectedPattern}. Missing patterns: ${result.missingPatterns.join(', ')}`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (result.hasErrors) {
      const errorMessages = result.errorLogs
        .map((log) => `${log.timestamp}: ${log.message}`)
        .join('\n');
      throw new Error(
        `Lambda function has ${result.errorCount} errors in logs:\n${errorMessages}`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (metrics.errors > 0) {
      throw new Error(
        `Lambda function has ${metrics.errors} errors in execution metrics`
      );
    }
    ...
    if (metrics.averageDuration > 5000) {
      // 5 seconds
      throw new Error(
        `Lambda function average duration ${metrics.averageDuration}ms is too slow`
      );
    }
  }
);
...
Then(
  ...
  async function (
    ...
    if (!result.matches) {
      throw new Error(
        `Step Function state ${stateName} output does not match expected data. Missing fields: ${result.missingFields.join(', ')}. Actual output: ${JSON.stringify(result.actualOutput)}`
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (dataFlow.dataCorruption) {
      throw new Error(
        'Step Function execution has data corruption between states'
      );
    }
  }
);
...
Then(
  ...
  async function (this: StepContext) {
    ...
    if (!result.meetsSLAs) {
      throw new Error(
        `Step Function execution does not meet SLAs: ${result.violations.join(', ')}`
      );
    }
  }
);
